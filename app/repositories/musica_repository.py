...
class MusicaRepository:
    ...
    def listar_todas(self, titulo=None, id_artista=None, genero=None, ano=None, pagina=1, limite=10, album=None):
        query = Musica.query
        if titulo:
    ...
    def adicionar_musica(self, dados_musica):
        # artista_nome = dados_musica["artista_nome"]
        # artista = Artista.query.filter_by(nome=artista_nome).first()
        # if not artista:
        #     artista = Artista(nome=artista_nome)
        #     db.session.add(artista.id)
        #     db.session.commit()
